...
void check_input(UI_Controller *controller) {
	...
	for (size_t i = 0; i < n_bindings; ++i) {
		if (IsKeyPressed(bindings[i].key))
			UI_Controller_notifyButton(controller, UI_CONTROLLER_BUTTON_PRESS, bindings[i].button);
		if (IsKeyReleased(bindings[i].key))
			UI_Controller_notifyButton(controller, UI_CONTROLLER_BUTTON_RELEASE, bindings[i].button);
	}
}
